(function( $, undefined ) {
		...
		mentionsDefaults = {
			callbacks: {
				...
				remote_filter: function( query, render_view ) {
					...
					params = { 'action': 'bp_get_suggestions', 'term': query, 'type': 'mla_members' };
	...
	$( document ).ready(function() {
		...
		loadMentionsInTinyMCE = function() {
			if ( loadAttempts < 4 || ! $( 'body' ).hasClass( 'forum' ) ) {
				loadAttempts++;
			...
			$( window.tinyMCE.activeEditor.contentDocument.activeElement )
				.atwho( 'setIframe', $( '#' + window.tinyMCE.activeEditor.iframeElement.id )[0] )
				.bp_mentions( users );
		};
